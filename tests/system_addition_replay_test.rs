use rust_ecs::{
    game::game::{RenderSystem, WaitSystem},
    World,
};
...
fn test_system_addition_recording_and_replay() {
    ...
    match operation {
        rust_ecs::WorldOperation::AddSystem(system_type) => {
            assert!(
                system_type.contains("WaitSystem"),
                "Should record WaitSystem addition"
            );
        }
    ...
    assert!(
        !update.system_diffs().is_empty(),
        "Update should have at least 1 system diff from the replayed WaitSystem"
    );
