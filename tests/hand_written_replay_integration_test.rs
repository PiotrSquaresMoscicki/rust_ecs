...
# End of replay log - Total updates: 5"#;
    ...
    println!(
        "✅ Parsed {} updates from hand-written replay file",
        replay_history.len()
    );
    ...
    assert_eq!(
        world.entities_with_component::<Position>().len(),
        3,
        "Should have 3 entities with Position components"
    );
    assert_eq!(
        world.entities_with_component::<Target>().len(),
        3,
        "Should have 3 entities with Target components"
    );
    assert_eq!(
        world.entities_with_component::<WaitTimer>().len(),
        3,
        "Should have 3 entities with WaitTimer components"
    );
    assert_eq!(
        world.entities_with_component::<Actor>().len(),
        3,
        "Should have 3 entities with Actor components"
    );
    assert_eq!(
        world.entities_with_component::<ActorState>().len(),
        3,
        "Should have 3 entities with ActorState components"
    );
...
fn verify_position(world: &World, entity: Entity, expected_x: i32, expected_y: i32, frame: &str) {
    if let Some(pos) = world.get_component::<Position>(entity) {
        assert_eq!(
            pos.x, expected_x,
            "{}: Entity {:?} x position should be {}, got {}",
            frame, entity, expected_x, pos.x
        );
        assert_eq!(
            pos.y, expected_y,
            "{}: Entity {:?} y position should be {}, got {}",
            frame, entity, expected_y, pos.y
        );
        println!(
            "✅ Entity {:?} position verified: ({}, {})",
            entity, pos.x, pos.y
        );
    } else {
        panic!(
...
fn verify_target(world: &World, entity: Entity, expected_x: i32, expected_y: i32, frame: &str) {
    if let Some(target) = world.get_component::<Target>(entity) {
        assert_eq!(
            target.x, expected_x,
            "{}: Entity {:?} target x should be {}, got {}",
            frame, entity, expected_x, target.x
        );
        assert_eq!(
            target.y, expected_y,
            "{}: Entity {:?} target y should be {}, got {}",
            frame, entity, expected_y, target.y
        );
        println!(
        ...
        panic!(
            "{}: Entity {:?} should have Target component",
            frame, entity
        );
    }
}
...
fn verify_wait_timer(world: &World, entity: Entity, expected_ticks: u32, frame: &str) {
    if let Some(timer) = world.get_component::<WaitTimer>(entity) {
        assert_eq!(
            timer.ticks, expected_ticks,
            "{}: Entity {:?} wait timer should be {}, got {}",
            frame, entity, expected_ticks, timer.ticks
        );
        println!(
            "✅ Entity {:?} wait timer verified: {}",
            entity, timer.ticks
        );
    } else {
        panic!(
            "{}: Entity {:?} should have WaitTimer component",
            frame, entity
        );
    }
}
