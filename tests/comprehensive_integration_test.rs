...
impl System for MovementSystem {
    ...
    fn update(&mut self, world: &mut WorldView<Self::InComponents, Self::OutComponents>) {
        ...
        for (entity, (position, velocity)) in world.query_components::<(Out<Position>, In<Velocity>)>() {
            position.0.x += velocity.0.dx;
            position.0.y += velocity.0.dy;
            println!("    Moved entity {:?} to ({:.1}, {:.1})", entity, position.0.x, position.0.y);
        }
    }
...
impl System for HealthSystem {
    ...
    fn update(&mut self, world: &mut WorldView<Self::InComponents, Self::OutComponents>) {
        self.frame_count += 1;
        println!("  [HealthSystem] Processing health (frame {})", self.frame_count);
        
        for (entity, health) in world.query_components::<(Out<Health>,)>() {
            if health.0.current < health.0.max {
                health.0.current = (health.0.current + 1).min(health.0.max);
                println!("    Regenerated health for entity {:?}: {}/{}", entity, health.0.current, health.0.max);
            }
        }
    }
...
impl System for CrossWorldSystem {
    ...
    fn update(&mut self, world: &mut WorldView<Self::InComponents, Self::OutComponents>) {
        ...
        for (entity, (position, velocity)) in world.query_components::<(In<Position>, In<Velocity>)>() {
            let speed = (velocity.0.dx * velocity.0.dx + velocity.0.dy * velocity.0.dy).sqrt();
            total_speed += speed;
            entity_count += 1;
            println!("    Analyzed entity {:?} at ({:.1}, {:.1}) with speed {:.2}", 
                    entity, position.0.x, position.0.y, speed);
        }
        
        if entity_count > 0 {
            println!("    Average speed across {} entities: {:.2}", entity_count, total_speed / entity_count as f32);
        }
    }
...
fn comprehensive_ecs_integration_test() {
    ...
    let child_world_id = main_world.create_child_world();
    {
        ...
        child_world.add_component(child_entity1, Health { current: 100, max: 100 });
        ...
        println!("Created child world {} with entities: {:?}, {:?}", 
    ...
    for frame in 1..=3 {
        ...
        if frame == 2 {
            println!("  [Special Action] Removing velocity from entity1 and adding health to entity3");
            main_world.remove_component::<Velocity>(entity1);
            main_world.add_component(entity3, Health { current: 50, max: 80 });
        }
    }
    
    // === PHASE 5: Entity Removal ===
    println!("\nPHASE 5: Demonstrating entity removal");
    println!("Before removal: {} entities exist", main_world.entity_count());
    main_world.remove_entity(entity2);
    println!("After removing entity2: {} entities exist", main_world.entity_count());
    ...
    println!("Successfully replayed {} updates in new world", history.updates.len());
    
    // Verify replay worked by checking some basic properties
    assert_eq!(replayed_world.entity_count(), main_world.entity_count());
    println!("✅ Replay verification: Entity counts match ({} entities)", replayed_world.entity_count());
...
fn visualize_world_history(history: &WorldUpdateHistory) {
    println!("World Update History ({} updates recorded):", history.updates.len());
    
    for (update_index, world_update) in history.updates.iter().enumerate() {
        println!("  Update {}: {} system updates", update_index + 1, world_update.system_diffs.len());
        ...
        for (system_index, system_diff) in world_update.system_diffs.iter().enumerate() {
            ...
            for change in system_diff.component_changes() {
                match change {
                    DiffableComponentChange::Added { entity, type_name, data } => {
                        println!("      Added {} to {:?}: {}", type_name, entity, data);
                    }
                    DiffableComponentChange::Modified { entity, type_name, diff } => {
