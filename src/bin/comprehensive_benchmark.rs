...
fn calculate_stats(durations: &[std::time::Duration]) -> BenchmarkStats {
    ...
    let avg_duration = std::time::Duration::from_nanos((total_nanos / durations.len() as u128) as u64);
...
fn print_benchmark_results(
    ...
    println!("├─────────────────────────┼─────────────┼─────────────┼─────────────┼─────────────┤");
    println!("│ No Recording            │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │",
    ...
    println!("│ Text Recording          │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │",
    ...
    println!("│ Binary Recording        │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │ {:>10.3}µs │",
    ...
    // Overhead Analysis
    let text_overhead_percent = calculate_overhead_percent(
        no_recording.avg_duration, 
        text_recording.avg_duration
    );
    let binary_overhead_percent = calculate_overhead_percent(
        no_recording.avg_duration, 
        binary_recording.avg_duration
    );
    let binary_vs_text_improvement = calculate_improvement_percent(
        text_recording.avg_duration,
        binary_recording.avg_duration
    );
    ...
    println!("├─────────────────────────┼─────────────┼─────────────┼─────────────┤");
    println!("│ Text vs No Recording    │ {:>10.1}% │ {:>10.1}x │ {:>10.1}µs │",
        text_overhead_percent,
        text_recording.avg_duration.as_nanos() as f64 / no_recording.avg_duration.as_nanos() as f64,
        (text_recording.avg_duration.as_nanos() - no_recording.avg_duration.as_nanos()) as f64 / (iterations as f64 * 1000.0)
    );
    println!("│ Binary vs No Recording  │ {:>10.1}% │ {:>10.1}x │ {:>10.1}µs │",
        binary_overhead_percent,
        binary_recording.avg_duration.as_nanos() as f64 / no_recording.avg_duration.as_nanos() as f64,
        (binary_recording.avg_duration.as_nanos() - no_recording.avg_duration.as_nanos()) as f64 / (iterations as f64 * 1000.0)
    );
    println!("│ Binary vs Text (impr.)  │ {:>10.1}% │ {:>10.1}x │ {:>10.1}µs │",
        -binary_vs_text_improvement,
        text_recording.avg_duration.as_nanos() as f64 / binary_recording.avg_duration.as_nanos() as f64,
        (text_recording.avg_duration.as_nanos() - binary_recording.avg_duration.as_nanos()) as f64 / (iterations as f64 * 1000.0)
    );
    println!("└─────────────────────────┴─────────────┴─────────────┴─────────────┘\n");
    ...
    println!("\nKey Findings:");
    println!("• Binary recording is {:.1}x faster than text recording", 
        text_recording.avg_duration.as_nanos() as f64 / binary_recording.avg_duration.as_nanos() as f64);
    println!("• Binary format reduces replay overhead by {:.1}% vs text format", binary_vs_text_improvement);
...
fn calculate_overhead_percent(baseline: std::time::Duration, with_overhead: std::time::Duration) -> f64 {
...
impl GameWorld {
    ...
    fn update_with_text_logging(&mut self, log: &mut TextReplayLog) {
        ...
        for actor in &mut self.actors {
            ...
            if old_position.x != actor.position.x || old_position.y != actor.position.y {
                log.record_component_change(actor.id, "Position", format!("({}, {})", actor.position.x, actor.position.y));
            }
            if old_target.x != actor.target.x || old_target.y != actor.target.y {
                log.record_component_change(actor.id, "Target", format!("({}, {})", actor.target.x, actor.target.y));
            }
        }
    ...
    fn update_with_binary_logging(&mut self, log: &mut BinaryReplayLog) {
        ...
        for actor in &mut self.actors {
            ...
            if old_position.x != actor.position.x || old_position.y != actor.position.y {
                log.record_component_change_binary(actor.id, "Position", &(actor.position.x, actor.position.y));
            }
            if old_target.x != actor.target.x || old_target.y != actor.target.y {
                log.record_component_change_binary(actor.id, "Target", &(actor.target.x, actor.target.y));
            }
        }
    ...
    fn update_actor(&self, actor: &mut Actor) {
