...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Print results
    println!("\n=== Results ===");
    println!("Original (full detail): {:.3}ms", original_time.as_secs_f64() * 1000.0);
    println!("Optimized (minimal):    {:.3}ms", optimized_time.as_secs_f64() * 1000.0);
    ...
    println!("Performance improvement: {:.1}% reduction in overhead", reduction);
    ...
    for i in 0..num_updates {
        ...
        for entity_id in 0..3 {
            ...
            movement_diff.add_diff_change(DiffComponentChange::Modified {
                ...
                diff_string: format!("PositionDiff {{ x: Some({}), y: Some({}) }}", 
                    (i + entity_id) % 10, 
                    (i + entity_id + 1) % 10
                ),
            });
...
fn benchmark_replay_logging(
    ...
    // Clean up test files
    let _ = std::fs::remove_dir_all(&logger.session_id().split('_').next().unwrap_or("replay_logs"));
