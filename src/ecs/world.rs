...
use crate::ecs::core::{ComponentChange, ComponentOperation, Entity, WorldOperation};
...
impl<I, O> WorldView<I, O> {
    ...
    pub fn query_position_actor(
        &self,
    ) -> Vec<(
        Entity,
        (
            &crate::game::components::Position,
            &crate::game::components::Actor,
        ),
    )> {
        unsafe {
            ...
            results
                .into_iter()
                .map(|(e, (pos1, actor, _pos2))| (e, (pos1, actor)))
                .collect()
        }
    }
    ...
    pub fn update_position(
        &mut self,
        entity: Entity,
        new_position: crate::game::components::Position,
    ) {
        unsafe {
    ...
    pub fn get_actors_for_movement(
        &self,
    ) -> Vec<(
        Entity,
        crate::game::components::Position,
        crate::game::components::Actor,
        crate::game::components::Target,
    )> {
        unsafe {
            let world_ref = &*self.world;
            ...
            results
                .into_iter()
                .map(|(e, (pos, actor, target))| (e, *pos, *actor, *target))
                .collect()
        }
    }

    /// Get entities with all components needed for wait system
    pub fn get_actors_for_wait_system(
        &self,
    ) -> Vec<(
        Entity,
        crate::game::components::Position,
        crate::game::components::Actor,
        crate::game::components::WaitTimer,
        crate::game::components::Target,
    )> {
    ...
    pub fn update_wait_timer(
        &mut self,
        entity: Entity,
        new_timer: crate::game::components::WaitTimer,
    ) {
        unsafe {
            let world_ref = &mut *self.world;
    ...
    pub fn query_position_actor_target_components(
        &self,
    ) -> Vec<(
        Entity,
        (
    ...
    pub fn record_component_modification<T: DiffComponent>(
        &mut self,
        entity: Entity,
        _old: &T,
        _new: &T,
...
impl World {
    ...
    pub fn query_multi_components<T1: 'static, T2: 'static, T3: 'static>(
        &self,
    ) -> Vec<(Entity, (&T1, &T2, &T3))> {
            ...
            for (entity, component1) in components1 {
                ...
                if let (Some(components2), Some(components3)) = (
                    ...
                    if let (Some(component2), Some(component3)) =
                        (components2.get(entity), components3.get(entity))
                    {
    ...
    pub fn query_actor_wait_components(
        &self,
    ) -> Vec<(
        Entity,
        crate::game::components::Position,
            ...
            for (entity, pos_component) in positions {
                ...
                if let (Some(actors), Some(timers), Some(targets)) = (
                    ...
                    if let (Some(actor_comp), Some(timer_comp), Some(target_comp)) =
                        (actors.get(entity), timers.get(entity), targets.get(entity))
                    {
                        // Downcast all components
    ...
    pub fn initialize_systems(&mut self) {
        ...
        let mut world_update_diff = WorldUpdateDiff::default();
