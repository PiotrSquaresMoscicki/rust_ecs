use rust_ecs::{
    game::{initialize_game, MovementSystem, RenderSystem, WaitSystem},
    World,
};
...
fn main() {
            ...
            for operation in system_diff.world_operations() {
                match operation {
                    rust_ecs::WorldOperation::AddSystem(system_type) => {
                        println!(
                            "     -> System addition: {}",
                            system_type.split("::").last().unwrap_or(system_type)
                        );
                    }
                    _ => {
    ...
    println!(
        "   - Fresh world update history length: {}",
        fresh_history.len()
    );
    ...
    println!(
        "   - initialize_game() recorded {} operations",
        game_history.len()
    );
